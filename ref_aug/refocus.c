...
/* RULING ARGUMENT */
#define	Alpha               prhs[7]
#define	V_begin               prhs[8]
#define	V_end               prhs[9]
#define	U_begin              prhs[10]
#define	U_end               prhs[11]
...
void remapping
    ...
    for (x = 0; x < width; ++x)
        for (y = 0; y < height; ++y)       //x,y表示重聚焦之后的空间坐标
        {
        output_color_R =0;
        output_color_G =0;
        output_color_B =0;
        ...
        for (i = v_begin; i < v_end+1; ++i)
            //for (j = -stereo_diff; j < stereo_diff+1; ++j)
            for (j = u_begin; j < u_end+1; ++j)
            {
            x_ind   = ((double)i)*(alpha) + ((double)x);  //这里用alpha替换1-1/alpha
            y_ind   = ((double)j)*(alpha) + ((double)y);  //x_ind,y_ind表示重聚焦前的空间域坐标
            
            x_floor = floor(x_ind);
            y_floor = floor(y_ind);
            
            x_1     = index_x(x_floor  ,width );          //双线性插值四周的空间坐标
            y_1     = index_y(y_floor  ,height);
            x_2     = index_x(x_floor+1,width );
            y_2     = index_y(y_floor+1,height);
            
            x_1_w   = 1-(x_ind-x_floor)        ;          //双线性插值权重
            x_2_w   = 1-x_1_w                  ;
            y_1_w   = 1-(y_ind-y_floor)        ;
            y_2_w   = 1-y_1_w                  ;
            
            x_1_index = i+stereo_diff + (x_1)*window_side   ;  //双线性插值四周的空间坐标对应的全光场坐标
            ...
            x_index_remap = i+stereo_diff + (x)*window_side   ;  //重聚焦之后的空间坐标对应的全光场坐标
            ...
            // DEFOCUS ANALYSIS
            output_color_R = interp_color_R + output_color_R;  //对重聚焦后同一空间位置处所有视角像素值进行累加
            output_color_G = interp_color_G + output_color_G;
            output_color_B = interp_color_B + output_color_B;
            
            }
        output_image[y + x * height + 0 * height*width] = output_color_R/window_size; //对累加后所有视角像素值取平均得到
        output_image[y + x * height + 1 * height*width] = output_color_G/window_size; //每一空间位置坐标的像素值
        output_image[y + x * height + 2 * height*width] = output_color_B/window_size; //output_image为重聚焦后的图片（512×512）
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[])
	...
	{ 
	double *x_size_fin_pt, *y_size_fin_pt, *window_side_pt, *stereo_diff_pt, *im_in_remap_pt, *im_out_remap_pt, *output_image_pt, *alpha_pt,*v_begin_pt,*v_end_pt,*u_begin_pt,*u_end_pt;
    ...
    v_begin_pt              = (double *) mxGetPr(V_begin)       ;
    v_end_pt                = (double *) mxGetPr(V_end)         ;
    u_begin_pt              = (double *) mxGetPr(U_begin)       ;
    u_end_pt                = (double *) mxGetPr(U_end)         ;
